...
export default function EditPage() {
  ...
  const params = useLocalSearchParams<{ id: string; title: string; amount: string; category: string }>()
  const { updateTransaction } = useTransactions(user?.id)

  const initialAmount = Math.abs(Number(params.amount))
  const initialType: TxType = Number(params.amount) >= 0 ? 'income' : 'expense'
  ...
  const handleSubmit = async () => {
    ...
    const success = await updateTransaction(Number(params.id), {
      title: title.trim(),
      amount: finalAmount,
      category,
    })
...
const styles = StyleSheet.create({
  ...
  content: { paddingHorizontal: S.containerMargin, paddingBottom: 48 },
  ...
  toggleBtn: { flex: 1, paddingVertical: 11, alignItems: 'center', borderRadius: R.full },
  toggleExpense: { backgroundColor: C.expense },
  toggleIncome:  { backgroundColor: C.income },
  toggleText: { fontSize: 15, fontWeight: '600', color: C.onSurfaceVariant },
  ...
  currency: { fontSize: 36, fontWeight: '700', paddingBottom: 8 },
  amountInput: {
    fontSize: 60, fontWeight: '700', color: C.onBackground,
    minWidth: 120, textAlign: 'left', letterSpacing: -1,
  },
  ...
  input: {
    ...
    paddingVertical: 14, fontSize: 16, color: C.onBackground,
  },
  ...
  catIconSelected: { backgroundColor: C.primaryContainer, borderColor: C.primaryContainer },
  catLabel: { fontSize: 10, fontWeight: '600', color: C.onSurfaceVariant, textAlign: 'center' },
  catLabelSelected: { color: C.primaryContainer, fontWeight: '700' },
